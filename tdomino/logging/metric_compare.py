...
def plot_multi_comparison(stat, obj_labels=None):
    ...
    line_style = ['-', ':', '-.', '--']
    C = [plt.cm.tab20c(c) for c in np.linspace(0.0,.4,n_metric)][::-1] # Colors
...
def plot_single_comparison(stat):
    ...
    C = [plt.cm.tab20c(c) for c in np.linspace(0.0,.4,n_alg)][::-1] # Colors
    ...
    for i, alg in enumerate(algs):
        ...
        y2 = stat[alg]['75th']
        x = np.arange(len(y0))+1
        ax.fill_between(x, y1, y2, color=C[i], alpha=0.2)
...
if __name__ == '__main__':
    exps = []
    exps += [Path('../../run/log/rast/Prod/')]
    exps += [Path('../../run/log/rast/Sum/')]

    qd = get_stats(exps, 'QD Score')
    a_size = get_stats(exps, 'Archive Size')

    host = plot_comparison(qd, obj_labels=None)
    host.set_title("QD Score")

    ax = plot_comparison(a_size, obj_labels=None)
